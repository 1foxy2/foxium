...
public interface LithiumTransferConditionInventory {
     ...
     * foxium's hopper optimization transfers a single item, but to avoid excessive copying of item stacks, it passes
     * the original stack to the inventory's insertion test. If the inventory requires a stack size of 1 for this test,
     * the stack should be copied. However, foxium cannot detect whether the copy is necessary and this method is meant
     * to signal this requirement. When the method is not implemented even though it is required, foxium's hopper
